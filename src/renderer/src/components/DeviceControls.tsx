...
export const DeviceControls: React.FC<DeviceControlsProps> = ({
  ...
  const [customCommandType, setCustomCommandType] = useState<"command" | "customize">(COMMAND_TYPE_COMMAND);
  ...
  const shouldConfirmCommand = (command: string) =>
    ...
    !isInfraredRemote &&
    (isBot || isPlug) &&
    (command === COMMAND_TURN_ON || command === COMMAND_TURN_OFF || command === COMMAND_PRESS);
  ...
  const sendCommand = (command: string, parameter: any = DEFAULT_PARAMETER, commandType: "command" | "customize" = COMMAND_TYPE_COMMAND) => {
  ...
  const sendHumidifierMode = (mode: EvaporativeModeKey | StandardHumidifierModeKey) => {
    if (isEvaporativeHumidifier) {
      ...
      if (mode in modeMap) {
        sendCommand(COMMAND_SET_MODE, { mode: modeMap[mode as EvaporativeModeKey], targetHumidify: safeTarget });
      }
      return;
    }

    const modeMap = { auto: "auto", low: "101", medium: "102", high: "103" };
    sendCommand(COMMAND_SET_MODE, modeMap[mode]);
  };
          ...
          {cmds.map((cmd) => (
            <Box key={cmd.command} sx={{ borderBottom: "1px dashed rgba(255,255,255,0.12)", pb: dense ? 0.5 : 1, mb: dense ? 0.5 : 1 }}>
              ...
              <Button
                ...
                disabled={controlsDisabled}
                onClick={() => sendCommand(cmd.command, resolveParameterValue(cmd), cmd.commandType || COMMAND_TYPE_COMMAND)}
              >
                {cmd.label}
              </Button>
            </Box>
          ...
          {remoteSupportsDefaultCommands && (
            <Box sx={buttonGridSx}>
              <Button
                ...
                onClick={() => {
                  if (isCeilingLight) {
                    sendCommand(COMMAND_SET_BRIGHTNESS, Math.round(clamp(brightness, 1, 100)));
                    return;
                  }
            ...
            <>
              ...
              <Stack spacing={1} sx={{ width: "100%", maxWidth: maxControlWidth }}>
                ...
                >
                  ...
                  <MenuItem value={COMMAND_TYPE_CUSTOMIZE}>customize (for user-defined buttons)</MenuItem>
                </TextField>
                <Button
                  variant="outlined"
                  size="small"
                  disabled={!customCommand || controlsDisabled}
                  onClick={() => sendCommand(customCommand, customParameter || DEFAULT_PARAMETER, customCommandType)}
                >
                  Send custom command
                </Button>
      ...
      <Stack spacing={dense ? 0.6 : 1.2}>
        {isBot && (
          <>
            ...
            {(botModeType === "press" || botModeType === "customize") ? (
              ...
              <Box sx={buttonGridSx}>
                {botShowsOnOff && (
                  <Button size="small" variant="contained" onClick={() => sendCommand(COMMAND_TURN_ON)} disabled={controlsDisabled} fullWidth>
                    Turn On
                  </Button>
                )}
                {botShowsOnOff && (
                  <Button size="small" variant="contained" color="secondary" onClick={() => sendCommand(COMMAND_TURN_OFF)} disabled={controlsDisabled} fullWidth>
                    Turn Off
                  </Button>
                )}
                {botShowsPress && (
                  <Button size="small" variant="contained" onClick={() => sendCommand(COMMAND_PRESS)} disabled={controlsDisabled} fullWidth>
                    Press
                  </Button>
                )}
              </Box>
        ...
        {(isCurtain) && (
          <>
            ...
            <Box sx={buttonGridSx}>
              <Button size="small" variant="contained" onClick={() => sendCommand(COMMAND_TURN_ON)} disabled={controlsDisabled} fullWidth>Open</Button>
              <Button size="small" variant="outlined" onClick={() => sendCommand("pause")} disabled={controlsDisabled} fullWidth>Pause</Button>
              <Button size="small" variant="contained" color="secondary" onClick={() => sendCommand(COMMAND_TURN_OFF)} disabled={controlsDisabled} fullWidth>Close</Button>
            </Box>
            <Box sx={sliderContainerSx}>
              <Slider
                size="small"
                ...
                onChangeCommitted={(_, value) => sendCommand(COMMAND_SET_POSITION, `0,ff,${clamp(value as number, 0, 100)}`)}
                valueLabelDisplay="auto"
            ...
            {supportsLightBrightness && (
              <Box sx={sliderContainerSx}>
                ...
                <Slider
                  ...
                  onChangeCommitted={(_, value) => sendCommand(COMMAND_SET_BRIGHTNESS, Math.round(value as number))}
                  valueLabelDisplay="auto"
            ...
            {supportsLightColorTemperature && (
              <Box sx={sliderContainerSx}>
                ...
                <Slider
                  ...
                  onChangeCommitted={(_, value) => sendCommand(COMMAND_SET_COLOR_TEMPERATURE, Math.round(value as number))}
                  valueLabelDisplay="auto"
          ...
          <>
            ...
            <Stack spacing={1} sx={{ width: "100%", maxWidth: maxControlWidth }}>
              ...
              <Button
                ...
                size="small"
                disabled={!customCommand || controlsDisabled}
                onClick={() => sendCommand(customCommand, customParameter || DEFAULT_PARAMETER)}
              >
